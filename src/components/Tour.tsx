type TourDate = {
  ...
  city: string;
  venue: string;
  tag: { label: string; variant: "coral" | "default" | "muted" | "aqua" };
  action: { label: string; href: string; variant: "btn" | "link" };
  soldOut?: boolean;
};
...
const dates: TourDate[] = [
  {
    date: "JUN 05",
    city: "New York, NY",
    venue: "Brooklyn Steel · Opening Night",
    tag: { label: "Few Left", variant: "coral" },
    action: { label: "Tickets ›", href: "#", variant: "btn" },
  },
  {
    date: "JUN 12",
    city: "Boston, MA",
    venue: "Royale",
    tag: { label: "On Sale", variant: "default" },
    action: { label: "Tickets ›", href: "#", variant: "btn" },
  },
  {
    date: "JUN 19",
  ...
  },
  {
    ...
    city: "Washington, DC",
    venue: "The Anthem",
    tag: { label: "Sold Out", variant: "muted" },
    action: { label: "Join waitlist", href: "#", variant: "link" },
    soldOut: true,
  },
  {
    date: "JUL 24",
    city: "Nashville, TN",
    venue: "Ryman Auditorium · Historic Stage",
    tag: { label: "Few Left", variant: "coral" },
    action: { label: "Tickets ›", href: "#", variant: "btn" },
  },
  {
    ...
    city: "Los Angeles, CA",
    venue: "The Wiltern",
    tag: { label: "Pre-Sale · Members", variant: "aqua" },
    action: { label: "Sign in to access", href: "/account", variant: "link" },
  },
];

const tagClass: Record<TourDate["tag"]["variant"], string> = {
      ...
      <div className="max-w-[1280px] mx-auto">
        ...
        <ul className="list-none m-0 p-0 border-t border-stroke">
          {dates.map((d) => (
            <li
              key={d.date + d.city}
              className={`grid grid-cols-[80px_1fr] md:grid-cols-[130px_1fr_140px_160px] grid-rows-[auto_auto] md:grid-rows-1 items-center gap-x-4 gap-y-2 py-7 border-b border-stroke transition-all duration-300 hover:bg-aqua/[0.03] hover:px-3 ${d.soldOut ? "opacity-70" : ""}`}
            >
              <span className="font-display text-[26px] text-aqua tracking-[0.04em]">{d.date}</span>
              ...
              <span
                className={`col-start-2 md:col-start-auto justify-self-start inline-flex px-[11px] py-[5px] border font-mono text-[10px] tracking-[0.12em] uppercase ${tagClass[d.tag.variant]}`}
              >
                {d.tag.label}
              </span>

              {d.action.variant === "btn" ? (
                <a
                  href={d.action.href}
